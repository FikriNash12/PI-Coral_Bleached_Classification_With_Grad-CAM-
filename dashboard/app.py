...
.badge {
    background: rgba(32,180,100,0.12);
    border: 1px solid rgba(32,180,100,0.3);
    color: #20b464;
...
.pred-healthy {
    background: linear-gradient(135deg, #0d2018, #071510);
    border: 1px solid #1a3d28;
    border-radius: 12px;
    padding: 1rem 1.4rem;
    text-align: center;
...
# Ganti dengan File ID Google Drive model kamu
GDRIVE_FILE_ID = "GANTI_DENGAN_FILE_ID_MODEL_KAMU"
...
# SAMPLE IMAGES
# Letakkan beberapa gambar .jpg dari test set
# di folder 'samples/' dalam repo kamu
# Format: {"path": "samples/nama.jpg", "label": "Healthy"/"Bleached"}
SAMPLE_IMAGES = [
    {"path": "samples/cs_healthy.jpg", "label": "Healthy"},
    {"path": "samples/noaa_healthy.jpg", "label": "Healthy"},
    {"path": "samples/cs_bleached.jpg", "label": "Bleached"},
    {"path": "samples/noaa_bleached.jpg", "label": "Bleached"},
]
...
<div class="metric-grid">
    ...
    <div class="metric-card">
        <div class="metric-val">0.78</div>
        <div class="metric-lbl">Recall Bleached</div>
    </div>
    <div class="metric-card">
        <div class="metric-val">9.2K</div>
        <div class="metric-lbl">Total Data Training</div>
    </div>
</div>
""", unsafe_allow_html=True)
    ...
    for i in range(0, len(SAMPLE_IMAGES), 2):
        ...
        for j, col in enumerate(cols):
            ...
            with col:
                ...
                with st.container():
                    st.markdown(f"**Sampel {idx+1}** — Ground Truth: `{sample['label']}`")
                    show_result(pred_label, pred_prob, bleach_pct, img, heatmap, superimposed)
                st.markdown("<hr class='divider'>", unsafe_allow_html=True)
else:
    st.info("📁 Letakkan gambar sampel di folder `samples/` dalam repo. Lihat komentar di kode untuk format yang dibutuhkan.")
