...
def page_metrics():
    ...
    <div class="info-box" style="border-left-color: #f59e0b; background: linear-gradient(135deg, #1f1505, #0a0f0d); border-top: 1px solid #453008; border-right: 1px solid #453008; border-bottom: 1px solid #453008;">
        ...
        <ul style="margin: 0; padding-left: 1.2rem; color: #a3b8a8; line-height: 1.7;">
            <li><b>Akurasi Model dan Karakteristik Prediksi:</b> Model klasifikasi Custom CNN memiliki tingkat akurasi data uji sebesar <b>85,51%</b> (Akurasi Validasi ~83,73%). Output bersifat probabilistik dengan distribusi kesalahan yang seimbang (Recall Healthy 0,87 dan Recall Bleached 0,83), namun tetap memiliki risiko <i>false positives</i> (198 kasus) dan <i>false negatives</i> (141 kasus).</li>
            <li><b>Ketergantungan pada Sinyal Warna & Pencahayaan (Color/Brightness Dependency):</b> Berdasarkan analisis Grad-CAM pada kasus galat, model cenderung mengandalkan sinyal rona warna putih/terang (<i>whiteness/brightness</i>) dibandingkan morfologi tekstur polip karang semata. Karang sehat yang secara alami berwarna pucat atau terkena pantulan cahaya kuat (<i>overexposed</i>) berpotensi memicu <i>False Positive</i> (terprediksi Bleached), sedangkan karang memutih pada kondisi perairan gelap atau keruh (<i>turbid/low-light</i>) berisiko memicu <i>False Negative</i> (terprediksi Healthy).</li>
            <li><b>Potensi Shortcut Pembelajaran Domain (Domain Confounding):</b> Terdapat perbedaan karakteristik visual yang signifikan antara dataset NOAA (rata-rata kecerahan 105,33; ketajaman 274,55) dan Coralscapes (rata-rata kecerahan 123,20; ketajaman 380,82). Perbedaan statistik ini berpotensi menjadi <i>shortcut feature</i> bagi model dalam membedakan kelas.</li>
            <li><b>Rekomendasi Pengembangan:</b> Diperlukan augmentasi warna dan kontras bawah air yang lebih adaptif (seperti penyesuaian <i>color jitter</i>, simulasi turbiditas, dan normalisasi pencahayaan), eksperimen <i>ablation</i> berbasis tekstur, serta pengujian terisolasi pada citra bawah laut dengan variasi kekeruhan air yang ekstrem.</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
...
def page_predict():
    ...
    <div class="info-box" style="border-left-color: #f59e0b; background: linear-gradient(135deg, #1f1505, #0a0f0d); border-top: 1px solid #453008; border-right: 1px solid #453008; border-bottom: 1px solid #453008;">
        ...
        <ul style="margin: 0; padding-left: 1.2rem; color: #a3b8a8; line-height: 1.6; font-size: 0.88rem;">
            <li><b>Akurasi Pengujian sekitar 85%:</b> Prediksi sistem didasarkan pada kalkulasi probabilitas fitur visual dengan akurasi data uji <b>85,51%</b>. Model tidak luput dari potensi kesalahan klasifikasi (<i>false positives / negatives</i>).</li>
